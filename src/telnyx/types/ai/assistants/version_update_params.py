...
class VersionUpdateParams(TypedDict, total=False):
    ...
    New integrations should prefer `tool_ids` over inline `tools`. On update, a sent
    `tool_ids` array fully replaces the assistant's attached shared tools; omit the
    field to leave them unchanged. Single-instance tool types are counted across
    inline `tools` and `tool_ids` combined, so attaching a shared tool of such a
    ...
    shared tools created with the AI Tools endpoints. On update, a sent `tools`
    array fully replaces the assistant's inline tools; omit the field to leave the
    inline tools unchanged. Each tool type except `function`, `webhook`, and
    `client_side_tool` allows at most one instance per assistant, counted across
    inline `tools` and shared `tool_ids` combined — sending a duplicate of such a
    type returns HTTP 400 with error code 10015. Responses merge shared tools into
    `tools` with `shared: true`; when updating, omit those tools from the `tools`
    array and manage them through `tool_ids` instead.
    """
