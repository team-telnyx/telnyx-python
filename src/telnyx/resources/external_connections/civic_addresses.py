...
class CivicAddressesResource(SyncAPIResource):
    ...
    def retrieve(
        ...
        *,
        ...
        return self._get(
            path_template("/external_connections/{id}/civic_addresses/{address_id}", id=id, address_id=address_id),
    ...
    def list(
        ...
        *,
        ...
        if not id:
            raise ValueError(f"Expected a non-empty value for `id` but received {id!r}")
        return self._get(
            path_template("/external_connections/{id}/civic_addresses", id=id),
            options=make_request_options(
